...
- Exactly one Story may be active. Multiple `active_story` values, or a value
  ...
  blocks `continue`.
- `source.*` stays `null` when no GitHub MCP server is configured for the
  project. That is the current state: the project is `local_only`. Do not
...
(registry key `workflow_history`). Separate from `.claude/logs/tool-usage.jsonl`
(tool telemetry).
